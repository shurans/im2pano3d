...
% or
% mex WarpMeshColor.cpp -L/opt/X11/lib/ -I/opt/X11/include/ -lOSMesa
% on mac:
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  mexPrintf("Step 1: setup off-screen binding \n");
  mexPrintf("Step 2 \n");
  // Step 2: Setup basic OpenGL setting
  glEnable(GL_DEPTH_TEST);  mexPrintf("GL_DEPTH_TEST \n");
  glDisable(GL_LIGHTING);  mexPrintf("GL_LIGHTING \n");
  glEnable(GL_CULL_FACE); mexPrintf("GL_CULL_FACE \n");
  glCullFace(GL_BACK);  mexPrintf("GL_BACK \n");
  glPolygonMode(GL_FRONT, GL_FILL); mexPrintf("GL_FRONT \n");
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); mexPrintf("GL_COLOR_BUFFER_BIT \n");
  //glClearColor(m_clearColor[0], m_clearColor[1], m_clearColor[2], 1.0f); // this line seems useless
  glViewport(0, 0, m_width, m_height);  mexPrintf("glViewport \n");
