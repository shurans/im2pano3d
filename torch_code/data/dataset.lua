...
local initcheck = argcheck{
   ...
   {name="pickbyclass",
    type="number",
    help="if sample training images by class"},
...
function dataset:initList(...)
   ...
   local lines,numoflines,maxPathLength = lines_from(self.paths[1]);
   maxPathLength = maxPathLength+string.len(self.dataPath) 
   self.imagePath:resize(numoflines, maxPathLength):fill(0)
   self.imageClass:resize(numoflines):fill(0)
   -- print(self.imagePath:size())

   local s_data = self.imagePath:data()
   for k=1,#lines do
       -- get 
       idx = string.find(lines[k],',')
       local imgpath_l,classId_l

       if idx then
         imgpath_l =  string.sub(lines[k],1,idx-1)
         classId_l   =  tonumber(string.sub(lines[k],idx+1))
       else
         imgpath_l =  lines[k]
         classId_l =  1
       end
       ...
       -- print(k..' ' .. classId)
       imgpath_l = self.dataPath..'/'..imgpath_l
       ffi.copy(s_data, imgpath_l)
       s_data = s_data + maxPathLength
       self.imageClass[k] = classId_l
   end
   ...
   -- classList one class  
   local allImageIndx = torch.range(1,numoflines)
   local numofClass = torch.max(self.imageClass);
   if numofClass ==0 then numofClass =1; end
   for i = 1,numofClass do
       self.classes[i] = i
       local  idx_class = allImageIndx[torch.eq(self.imageClass,i)]
       -- print(idx_class:nElement())
       self.classList[i] = idx_class
   end
...
function dataset:getByList()
  local index = 0
  if self.serial_batches == 1 then 
    index = math.fmod(self.image_count-1, self.numSamples)+1
  else
    index = torch.random(1, self.numSamples)
  end
  self.image_count = self.image_count +1
  imgpath = ffi.string(torch.data(self.imagePath[index]))
  imgclass = self.imageClass[index]
  return self:sampleHookTrain(imgpath), imgpath,imgclass
end
-- getByClass
function dataset:getByClass(class)
   local index = math.ceil(torch.uniform() * self.classListSample[class]:nElement())
   local imgpath = ffi.string(torch.data(self.imagePath[self.classListSample[class][index]]))
   imgclass = self.imageClass[self.classListSample[class][index]]
   return self:sampleHookTrain(imgpath), imgpath,imgclass
end
...
function dataset:sample(quantity)
   ...
   local samplePaths = {}
   for i=1,quantity do
      local out, imgpath, imgclass
      if self.pickbyclass == 1 and self.serial_batches == 0 then
         local class = torch.random(1, #self.classes)
         out, imgpath, imgclass = self:getByClass(class)
      else
         out, imgpath, imgclass = self:getByList()
      end
      table.insert(dataTable, out)
      table.insert(scalarTable, imgclass)
